...
async function demonstrateEnhancedStructure() {
  try {
    ...
    // Search tables
    console.log('\n📋 Tables containing "Bank":');
    const bankTables = await enhancedTableMetadataService.searchEnhancedTables('Bank', 5);
    bankTables.forEach(table => {
      console.log(`📁 ${table.table_name}: ${table.table_label}`);
      if (table.table_description) {
        console.log(`   ${table.table_description}`);
      }
    });
    
    // Search fields
    console.log('\n📋 Fields containing "Type":');
    const typeFields = await enhancedTableMetadataService.searchEnhancedFields('Type', 5);
    ...
    // Search enum values
    console.log('\n📋 Enum values containing "Yes":');
    const yesEnums = await enhancedTableMetadataService.searchEnumValues('Yes', 3);
    yesEnums.forEach(enumVal => {
      console.log(`🔢 ${enumVal.table_name}.${enumVal.field_name}[${enumVal.enum_value}]: ${enumVal.enum_label}`);
    });
    
    // 4. Get specific field with enum values
    console.log('\n🎯 Specific Field Example - BankGroup.BankType_RU:');
    console.log('================================================');
